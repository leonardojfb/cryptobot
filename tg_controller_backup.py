...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await q.answer()
        log.debug(f"[CALLBACK] q.answer() ejecutado correctamente para '{data}'")
    except Exception as e:
        log.error(f"[CALLBACK] Error en q.answer(): {e}")
    ...
    try:
        ...
        # ── Actualizar posición ───────────────────────────────────────────────
            ...
            try:
                ...
                pass  # "message is not modified" — no hay cambio real, ignorar
        ...
        try:
            await q.edit_message_text(f"❌ Error interno: {e}")
        except Exception:
            pass
...
def run_telegram_bot(bot_instance):
    if not TG_AVAILABLE:
        log.error("python-telegram-bot no instalado.")
        return
    if not TG_TOKEN:
        log.warning("TELEGRAM_BOT_TOKEN vacío — Telegram desactivado.")
        return
